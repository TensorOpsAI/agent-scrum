...
class WorkflowOrchestrator:
    ...
    TASK_HANDLERS: dict[str, tuple | None] = {
        "draft": None,
        "pending_review": None,
        "in_review": None,
        "ready_for_development": ("developer", "implementation_notes"),
        "in_progress": None,
        "code_review": ("code_reviewer", "review_implementation"),
        "code_review_in_progress": None,
        "ready_for_qa": ("qa", "create_test_scenarios"),
        "qa_in_progress": ("qa", "run_tests"),
        "done": None,
    }
    ...
    async def process_story_transition(
        ...
        agent_role, skill = handler[0], handler[1]
    ...
    async def process_task_transition(
        ...
        template_id = board.get("template_id", "software_dev")
        workflow = TEMPLATE_WORKFLOWS.get(template_id, {})
        handler = workflow.get("task_handlers", {}).get(new_status)
